...
void Camara3Modos::desplRotarXY( const float da, const float db )
{
  ...
  float coef = 0.2; // coefificente para que vaya más suave la cámara
  ...
  // ---
   ...
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
        ...
        org_polares[0] += da*coef ;
        org_polares[1] += db*coef ;
      ...
      case ModoCam::prim_pers_despl :
      {
        ...
        punto_atencion[0] += da*coef;
        punto_atencion[1] += db*coef; 
...
void Camara3Modos::moverZ( const float dz )
{
   ...
   switch( modo_actual )
   {
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         // nota: los ejes no cambian
         // .....
        punto_atencion = punto_atencion + dz * eje[2];  //HE PUESTO +DS
