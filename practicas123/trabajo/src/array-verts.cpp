...
void DescrTabla::activar_mi()
{
   ...
   //
   // .....
   fijarPuntero( datos ); //datos puntero con la memoria del primer byte
...
void DescrTabla::activar_md()
{
   ...
   if (nombre_vbo == 0 )
     {
       glGenBuffers(1, &nombre_vbo);  // crea un nuevo nombre de VBO
       glBindBuffer( tipo_tabla, nombre_vbo ); //actvar el nuevo VBO
...
void ArrayVertices::visualizarGL_MI_BVE( const GLenum tipo_primitiva ) 
{
   ...
   // pag 96 diapositivas
   /**
 GLenum es un entero sin signo que codifica el número de primitiva
 Esta puede ser GL_TRIANGES, GL_POINTS, GL_LINES, GL_LINE_STRIP, GL_LINE_LOOP...) 
   */
   ...
   for ( GLuint i = 0 ; i < nv; i++ )
   {
     ...
     // enviar el vértice que corresponda
     ...
     // un ejemplo de valor a pasarle sería GLfloat coords[3] = { 3.1, 3.2, 9.6 }: 
...
void ArrayVertices::visualizarGL_MI_DAE( const GLenum tipo_primitiva ) 
{
   ...
   glBindVertexArray( 0);  //activa el VAO (vector array objecc)
   deshabilitar_tablas(); //deshabilitar todas la tablase  ( qu no se muestre lo anterior puede ser
   coordenadas-> activar_mi(); // activar la tabla de coordenadas

   //antivar las tablas de atributos que corresponda (en modo inmediato)
   ...
   //invocar 'glDrawElement o 'glDrawArray' en funcijón de si hay o no índices
   ...
   //deja tablas deshabilitada para visualiza otros arrays de vértices
...
void ArrayVertices::visualizarGL_MD_VAO( const GLenum tipo_primitiva ) 
{
   ...
   //activas la tablas de atributos que corresponda ( en modo inmediato)
